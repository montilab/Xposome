...
observeEvent(input$SignInButton, {
  ...
  logInfo <- read.csv(paste0("data/User_Login_List.csv"), header = TRUE) %>% arrange(Firstname)
  
  ## Verify username and password ###
  Log <- logInfo %>% filter(Username %in% userName, Password %in% passWord)
  
  ## Read in the project list ####
  projectInfo <- read.csv(paste0("data/Project_List.csv"), header = TRUE) %>% arrange(Project)
...
output$projecttable <- DT::renderDataTable({
  ...
  if(all(!is.na(projectdata()$Project))){
    table <- projectdata() %>% arrange(Project) %>% select(all_of(table_colnames))
  }else{
    table <- projectdata() %>% mutate_all(~replace(., is.na(.), "<br>")) %>% arrange(Project) %>% select(all_of(table_colnames))
  }  
      ...
      fluidRow(
        column(
          ...
          conditionalPanel(
            ...
            fileInput(inputId="edit_intro_file", label=strong(span(style="color:red;", "*"), "Choose an introduction file ", downloadButton(outputId = "edit_intro_download_data", label = em(style="font-size: 11px", "Introduction_page.Rmd")))),
          ...
          conditionalPanel(
            ...
            conditionalPanel(
              ...
              fileInput(inputId="edit_pro_file", label=strong(span(style="color:red;", "*"), "Choose a profile annotation file", downloadButton(outputId="edit_pro_download_data", label=em(style="font-size: 11px", "Profile_annotation.RDS")))),
              fileInputRadioButtonsWithTooltip(
          ...
          conditionalPanel(
            ...
            fileInput(inputId = "edit_ge_file", label = strong(span(style="color:red;", "*"), "Choose a gene expression file", downloadButton(outputId="edit_ge_download_data", label=em(style="font-size: 11px", "Expression_set.RDS")))),
            fileInputRadioButtonsWithTooltip(
        ...
        column(
          ...
          conditionalPanel(
            condition = "input.edit_files == 'Profile Annotation' | input.edit_files == 'Gene Expression' | input.edit_files == 'All' | input.edit_files == 'K2Taxonomer' | (input.edit_files == 'Connectivity Map' && input.edit_conn_pro_option == 'Yes')",
          ...
          conditionalPanel(
            ...
            conditionalPanel(
              condition = "input.edit_files == 'K2Taxonomer' | (input.edit_files == 'Connectivity Map' && input.edit_conn_pro_option == 'Yes') | (input.edit_files == 'All') | (input.edit_files == 'Profile Annotation') |  (input.edit_files == 'Gene Expression')",
...
observeEvent(input$Add_User_Add_Button, {
  ...
  if(Firstname=="" | Lastname=="" | Username=="" | Password==""){
    ...
    if(nrow(validate_user) > 0){
      ...
      logindata(newlogin)
      login_table_message(paste0(Username, ' ', Password, ' has been added. Click "Save" to keep the changes.'))
      removeModal()
    }
...
output$download_gs_collection_file <- downloadHandler(
  ...
  content = function(file){
    file.copy(paste0("data/Enrichment Gene Set/", portal()$GS_Collection), file)
  },
