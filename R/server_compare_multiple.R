...
observeEvent(input$compareGo, {
  ...
  taxonomer_results() %...>% {
    ...
    withProgress(message = "Comparing Selected Groups:", value = 0, {
      ...
      # Change column names
      colnames(EnrRes)  <- c("Gene Set", "NodeGroup", "P Value_Hyper", "FDR_Hyper", "N_Overlap", "N_Sig. Genes", "N_Gene Set", "P Value_ssGSEA", "FDR_ssGSEA", "Diff_ssGSEA", "Mean_ssGSEA", "Hits", "Link")
      ...
      clusterRes <- clusterRes[,c("gene", "NodeGroup", "pval", "fdr",  "coef", "mean", "Link")]
      ...
      # Rename columns
      colnames(clusterRes) <- c("Gene", "NodeGroup", "P Value", "FDR", "Diff", "Mean", "Link")
...
observeEvent(input$DGEmulti_rows_selected, {

  req(input$DGEmulti_rows_selected)
  
  # Get values####
  groupList = values() %>% extract2("groupList")
  rowNum = input$DGEmulti_rows_selected; 
  dgeTable = cluster_compare_results() %>% extract2("dgeTable")
...
observeEvent(input$HEmulti_rows_selected, {
  
  req(input$HEmulti_rows_selected)
  
  # Get values####
  groupList = values() %>% extract2("groupList")
  rowNum = input$HEmulti_rows_selected
