...
observeEvent(input$DGE_rows_selected, {
  
  req(input$DGE_rows_selected)
  
  filter = input$DGE_search_columns; 
  row = input$DGE_rows_selected; 
  all_rows = input$DGE_rows_all; 
  ...
  taxonomer_results() %...>% {
    ...
    if(all(filter == "")){
      ...
      dgeTable = .[["dgeTable"]][all_rows,]
    }
    
    rowNum <- as.numeric(row)
...
observeEvent(input$HE_rows_selected, {
  
  req(input$HE_rows_selected)
  
  filter = input$HE_search_columns; 
  row = input$HE_rows_selected; all_rows = input$HE_rows_all; 
