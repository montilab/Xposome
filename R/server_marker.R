...
running_marker_plot <- reactiveVal(FALSE);
...
observeEvent(input$de_generate, {
  ...
  if(running_marker_plot()) return(NULL)
  
  ##Allow the function to run####
  running_marker_plot(TRUE);
  ...
  results <- finally(results, function(){
    progress$close()
    running_marker_plot(FALSE) #declare done with run
  })
...
observeEvent(input$cancel_de_generate, {
  
  req(running_marker_plot())
  
  if(running_marker_plot())
    interruptor$interrupt("Process is canceled!")
...
##Observe when cancel button is clicked####
observeEvent(input$cancel_gs_generate, {
...
##Observe when cancel button is clicked####
observeEvent(input$cancel_conn_generate, {
