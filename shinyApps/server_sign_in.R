...
observeEvent(input$sign_out_btn, {
  ...
  ## Create a project list if it is empty ####
  if(nrow(projectdata()) == 0){
    ...
    ## Create a user login list if it is empty ####
    if(nrow(logindata())==0){
      ...
      write.csv(data, "www/data/User_Login_List.csv", row.names=FALSE)
...
observeEvent(input$Save_Project_Yes_Button, {
  ...
  if(!is.null(portal)){
    data <- projectdata() %>% full_join(read_csv(paste0("www/data/Project_List.csv")) %>% filter(!Portal %in% portal$Portal))
    ...
    data <- projectdata() %>% full_join(read_csv(paste0("www/data/Project_List.csv")))
  }
  
  project_table_message("Project list has been saved.")
  write.csv(data, paste0("www/data/Project_List.csv"), row.names = FALSE)
  projectdata(data)
  removeModal()
...
observeEvent(input$Add_User_Add_Button, {
  ...
  login_dat <- read.csv(paste0("www/data/User_Login_List.csv"), header=TRUE)                                       
...
observeEvent(input$Save_User_Yes_Button, {
  ...
  if(is.null(user)){
    data <- logindata() %>% dplyr::full_join(read.csv(paste0("www/data/User_Login_List.csv"), header=TRUE))
  }else{
    data <- logindata() %>% dplyr::full_join(read.csv(paste0("www/data/User_Login_List.csv"), header=TRUE) %>% filter(!Username %in% user$Username))           
  }
  
  login_table_message("Login list has been saved.")
  write.csv(data, paste0("www/data/User_Login_List.csv"), row.names = FALSE)
  logindata(data)
  removeModal()
...
output$add_intro_template_rmd <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/introduction.Rmd", file)
  },
...
output$add_pro_template_csv <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/profile_annotation.csv", file)
  },
...
output$add_pro_template_rds <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/profile_annotation.RDS", file)
  },
...
output$add_ge_template_csv <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/gene_expression.csv", file)
  },
...
output$add_ge_template_rds <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/gene_expression.RDS", file)
  },
...
output$add_conn_pcl_template_csv <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/connectivity_pcl.csv", file)
  },
...
output$add_conn_pcl_template_rds <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/connectivity_pcl.RDS", file)
  },
...
output$add_conn_pert_template_csv <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/connectivity_pert.csv", file)
  },
...
output$add_conn_pert_template_rds <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/connectivity_pert.RDS", file)
  },
...
output$add_gs_collection_template_gmt <- downloadHandler(
  ...
  content = function(file){
    file.copy("www/data/Template/hallmark.gmt", file)
  },
...
output$download_gs_collection_file <- downloadHandler(
  ...
  content = function(file){
    file.copy(paste0("www/data/Enrichment Gene Set/", portal()$GS_Collection, ".gmt"), file)
  },
