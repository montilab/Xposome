...
output$exposure_phenotype_plot <- renderPlotly({
  ...
  if(length(exposure_phenotype) == 1){
    ...
    ml <- create_marker_plot()[[exposure_phenotype]]
      
  }else{
    
    pl <- lapply(1:length(exposure_phenotype), function(x) { 
      legend_header <- exposure_phenotype[x]
      fig <- create_marker_plot()[[exposure_phenotype[x]]]
      fig <- fig %>% layout(legend = l(legend_header), hoverlabel = list(bgcolor="white"))
      fig
    })
    
    ml <- marrangeGrob(pl, ncol=1, nrow=length(exposure_phenotype))
