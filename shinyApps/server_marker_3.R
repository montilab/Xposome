...
create_marker_exposure_plot <- reactiveVal(NULL);
create_marker_table <- reactiveVal(NULL);
...
observeEvent(input$portal_id, {
  ...
  create_marker_exposure_plot(NULL)
  create_marker_table(NULL)
...
output$exposure_phenotype_plot <- renderPlotly({
  ...
  req(create_marker_plot(), exposure_phenotype())
  ...
  if(length(exposure_phenotype) == 1){
    ...
    fig <- create_marker_plot()[[exposure_phenotype]]
    fig <- fig %>% layout(legend = l(exposure_phenotype), hoverlabel = list(bgcolor="white"))

  }else{
    
    p1 <- create_marker_plot()[[exposure_phenotype[1]]] 
    p2 <- create_marker_plot()[[exposure_phenotype[2]]] 
    fig <- align_plots(p1, p2, align="v")
    fig <- fig %>% layout(legend = l(exposure_phenotype[1]), hoverlabel = list(bgcolor="white"))
...
output$marker_table_header <- renderUI({
  ...
  req(create_marker_plot(), marker_header())
